...
function rock_valid_moves(figures, id, i) {
    let moves = get_valid_vertical_moves(figures, id, i);
    moves = moves.concat(get_valid_horizontal_moves(figures, id, i));
    return moves;
}
...
function bishop_valid_moves(figures, id, i) {
    return get_valid_diagonal_moves(figures, id, i);
}

function queen_valid_moves(figures, id, i) {
    return get_valid_diagonal_moves(figures, id, i)
        .concat(get_valid_horizontal_moves(figures, id, i))
        .concat(get_valid_vertical_moves(figures, id, i));
}

function king_valid_moves(figures, id, i) {
    let vertical_moves = get_valid_vertical_moves(figures, id, i);
    let horizontal_moves = get_valid_horizontal_moves(figures, id, i);
    let diagonal_moves = get_valid_diagonal_moves(figures, id, i);

    let moves = [];
    moves = moves.concat(vertical_moves.filter(index => index < i).reverse()[0]);
    moves = moves.concat(vertical_moves.filter(index => index > i)[0]);
    moves = moves.concat(horizontal_moves.filter(index => index < i).reverse()[0]);
    moves = moves.concat(horizontal_moves.filter(index => index > i)[0]);
    moves = moves.concat(diagonal_moves.filter(index => index < i && index % 9 === i % 9).reverse()[0]);
    moves = moves.concat(diagonal_moves.filter(index => index < i && index % 7 === i % 7).reverse()[0]);
    moves = moves.concat(diagonal_moves.filter(index => index > i && index % 7 === i % 7)[0]);
    moves = moves.concat(diagonal_moves.filter(index => index > i && index % 9 === i % 9)[0]);

    return moves.filter(index => index != null);
}
...
function get_valid_vertical_moves(figures, id, i) {
    let vertical_moves = get_vertical_moves(i);
    let moves_by_direction = {
        n: vertical_moves.filter(index => index < i),
        s: vertical_moves.filter(index => index > i),
    };

    // front
    let nearest_figure = get_nearest_figure(moves_by_direction.n, figures, i);
    let moves = nearest_figure != null ? moves_by_direction.n
            .filter(index => Math.abs(figures[nearest_figure].id - id) < 16 ? index > nearest_figure : index >= nearest_figure) :
        moves_by_direction.n;

    // back
    nearest_figure = get_nearest_figure(moves_by_direction.s, figures, i);
    moves = nearest_figure!=null ? moves.concat(moves_by_direction.s
            .filter(index => Math.abs(figures[nearest_figure].id - id) < 16 ? index < nearest_figure : index <= nearest_figure)) :
        moves.concat(moves_by_direction.s);

    return moves;
}

function get_valid_horizontal_moves(figures, id, i){
    let horizontal_moves = get_horizontal_moves(i);
    let moves_by_direction = {
        w: horizontal_moves.filter(index => index < i),
        e: horizontal_moves.filter(index => index > i),
    };

    // left
    let nearest_figure = get_nearest_figure(moves_by_direction.w, figures, i);
    let moves = nearest_figure != null ? moves_by_direction.w
            .filter(index => Math.abs(figures[nearest_figure].id - id) < 16 ? index > nearest_figure : index >= nearest_figure) :
        moves_by_direction.w;

    // right
    nearest_figure = get_nearest_figure(moves_by_direction.e, figures, i);
    moves = nearest_figure != null ? moves.concat(moves_by_direction.e
            .filter(index =>  Math.abs(figures[nearest_figure].id - id) < 16 ? index < nearest_figure : index <= nearest_figure)) :
        moves.concat(moves_by_direction.e);

    return moves;
}

function get_valid_diagonal_moves(figures, id, i){
    let diagonal_moves = get_diagonal_moves(i);
    let moves_by_directions = {
        nw: diagonal_moves.filter(index => index < i && index % 9 === i % 9),
        ne: diagonal_moves.filter(index => index < i && index % 7 === i % 7),
        se: diagonal_moves.filter(index => index > i && index % 9 === i % 9),
        sw: diagonal_moves.filter(index => index > i && index % 7 === i % 7),
    };

    // north-west
    let nearest_figure = get_nearest_figure(moves_by_directions.nw, figures, i);
    let moves = nearest_figure ? moves_by_directions.nw
            .filter(index => Math.abs(figures[nearest_figure].id - id) < 16 ? index > nearest_figure : index >= nearest_figure)
        : moves_by_directions.nw;

    // north-east
    nearest_figure = get_nearest_figure(moves_by_directions.ne, figures, i);
    moves = nearest_figure ? moves
            .concat(moves_by_directions.ne
                .filter(index => Math.abs(figures[nearest_figure].id - id) < 16 ? index > nearest_figure : index >= nearest_figure))
        : moves.concat(moves_by_directions.ne);

    // south-east
    nearest_figure = get_nearest_figure(moves_by_directions.se, figures, i);
    moves = nearest_figure ? moves
            .concat(moves_by_directions.se
                .filter(index => Math.abs(figures[nearest_figure].id - id) < 16 ? index < nearest_figure : index <= nearest_figure))
        : moves.concat(moves_by_directions.se);

    // south-west
    nearest_figure = get_nearest_figure(moves_by_directions.sw, figures, i);
    moves = nearest_figure ? moves
            .concat(moves_by_directions.sw
                .filter(index => Math.abs(figures[nearest_figure].id - id) < 16 ? index < nearest_figure : index <= nearest_figure))
        : moves.concat(moves_by_directions.sw);
